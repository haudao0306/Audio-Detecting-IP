module features_layer2 #(
    ...
    // Input tu Layer 1: 16 channels x 16-bit
    input  wire                     in_valid,
    output wire                     in_ready,
    input  wire [IN_BUS_WIDTH-1:0]  in_pixels_flat,

    // Output sau Conv2 + MaxPool2: 32 channels x 16-bit
    output wire [OUT_BUS_WIDTH-1:0] out_pixels_flat,
    output wire                     out_valid
);

    // =====================================================
    // Internal connection: Conv2 -> MaxPool2
    // =====================================================
    wire [OUT_BUS_WIDTH-1:0] conv_out_data;
    wire                     conv_out_valid;

    // MaxPool2 hien tai khong co backpressure, nen luon ready
    wire conv_out_ready;
    ...
    maxpool2_top #(
        ...
        .in_pixels_512b(conv_out_data),

        .out_pixels_512b(out_pixels_flat),
        .out_valid(out_valid)
    );
