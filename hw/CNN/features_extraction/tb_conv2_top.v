...
module tb_conv2_top;
    ...
    localparam OUT_CHANNELS = 32;

    localparam IN_BUS_WIDTH  = IN_CHANNELS  * DATA_WIDTH; // 256
    localparam OUT_BUS_WIDTH = OUT_CHANNELS * DATA_WIDTH; // 512
    ...
    integer buffer_valid_cnt;
    integer dp_valid_cnt;
    ...
    initial begin
        $display("");
        $display("====================================================");
        $display(" LOADING TEST VECTORS FOR CONV2_TOP");
        $display("====================================================");

        $readmemh("conv2_top_input.hex", in_mem);
        $readmemh("conv2_top_output.hex", ref_mem);

        $display(" INPUT  VECTOR : conv2_top_input.hex");
        $display(" GOLDEN VECTOR : conv2_top_output.hex");
    ...
    initial begin
        ...
        $display("");
        $display("====================================================");
        $display(" START SENDING CONV2 INPUT STREAM");
        $display("====================================================");
        $display("");
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            if (dut.buffer_out_valid)
                buffer_valid_cnt = buffer_valid_cnt + 1;

            if (dut.dp_partial_valid)
                dp_valid_cnt = dp_valid_cnt + 1;
        end
    end
    ...
    begin
        ...
        $display(" BUFFER VALID COUNT : %0d", buffer_valid_cnt);
        $display(" DP PARTIAL COUNT   : %0d", dp_valid_cnt);
        $display(" OUTPUT COUNT       : %0d / %0d", out_cnt, TOTAL_PIXELS);
        $display(" TOTAL ERRORS       : %0d", error_cnt);

        if ((error_cnt == 0) && (out_cnt == TOTAL_PIXELS)) begin
            $display("");
            $display(" PASS: CONV2_TOP IS CORRECT");
        end
        else begin
            $display("");
            $display(" FAIL: CONV2_TOP HAS ERRORS OR MISSING OUTPUTS");
        end
