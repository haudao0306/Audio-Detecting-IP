module conv_buffer #(
    ...
    output reg  [DATA_WIDTH-1:0] p0, p1, p2,
    output reg  [DATA_WIDTH-1:0] p3, p4, p5,
    output reg  [DATA_WIDTH-1:0] p6, p7, p8,
    ...
    reg [DATA_WIDTH-1:0] mem [0:2][0:IMG_H-1];
    ...
    wire [6:0] max_ahead = (out_c == 0) ? 7'd3 : 7'd2;
    wire [6:0] col_distance = wr_col - out_c;
    ...
    wire [5:0] safe_r_up = (out_r == 0) ? 6'd0 : out_r - 1'b1;
    wire [5:0] safe_r_dn = (out_r == IMG_H - 1) ? out_r : out_r + 1'b1;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (consume_last_window) begin
                ...
                p0 <= (out_r == 0 || out_c == 0)             ? {DATA_WIDTH{1'b0}} : mem[l_ptr][safe_r_up];
                p1 <= (out_r == 0)                           ? {DATA_WIDTH{1'b0}} : mem[mid_ptr][safe_r_up];
                p2 <= (out_r == 0 || out_c == IMG_W - 1)     ? {DATA_WIDTH{1'b0}} : mem[r_ptr][safe_r_up];
    
                p3 <= (out_c == 0)                           ? {DATA_WIDTH{1'b0}} : mem[l_ptr][out_r];
                p4 <=                                                               mem[mid_ptr][out_r];
                p5 <= (out_c == IMG_W - 1)                   ? {DATA_WIDTH{1'b0}} : mem[r_ptr][out_r];
    
                p6 <= (out_r == IMG_H - 1 || out_c == 0)     ? {DATA_WIDTH{1'b0}} : mem[l_ptr][safe_r_dn];
                p7 <= (out_r == IMG_H - 1)                   ? {DATA_WIDTH{1'b0}} : mem[mid_ptr][safe_r_dn];
                p8 <= (out_r == IMG_H - 1 || out_c == IMG_W - 1)
                                                                ? {DATA_WIDTH{1'b0}} : mem[r_ptr][safe_r_dn];
